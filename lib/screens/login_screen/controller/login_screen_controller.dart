...
class LoginScreenController extends GetxController {
  ...
  late LoginOptionsScreenController loginOptionsScreenController;
  int? option;
  Rx<User> patient =
      User(0, "", "", "", "", "", "", "", "", "", "", false, "", 0, "").obs;
  ...
  logout() {
    ...
    PrefsService.to.remove(ConstRes.isArabicTop);
  ...
  login(UserCredintals userCredintals) async {
    ...
    if (res.token != null) {
      ...
      if (PrefsService.to.getString(ConstRes.afterLoginKey) == "file0") {
        Get.offAndToNamed(ConstRes.appointmentsList);
      } else if (PrefsService.to.getString(ConstRes.afterLoginKey) == "home0" ||
          PrefsService.to.getString(ConstRes.afterLoginKey) == "confirm") {
        Get.offAndToNamed(PagesNames.confirmReservation);
      } else if (PrefsService.to.getString(ConstRes.afterLoginKey) ==
              "family" ||
          PrefsService.to.getString(ConstRes.afterLoginKey) == "browseFile") {
        Get.offAndToNamed(PagesNames.main);
      } else {
  ...
  @override
  void onInit() {
    loginOptionsScreenController = Get.put(LoginOptionsScreenController());
    super.onInit();
  }
}
